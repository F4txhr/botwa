const { youtube, ttdl, igdl, aio } = require("btch-downloader");

/**
 * Cari string URL pertama di dalam object (rekursif) yang tampak seperti http/https.
 */
...
function pickBestVideoUrlFromBtch(data) {
  ...
  // Fallback terakhir: cari string yang tampak seperti URL di seluruh object
  const anyUrl = findAnyUrl(data);
  if (anyUrl) {
    return anyUrl;
  }
...
async function downloadBtchVideo(url) {
  ...
  if (/youtu\.be|youtube\.com/i.test(url)) {
    ...
    // Fallback: pakai AIO (auto detect)
    data = await aio(url);
  }
