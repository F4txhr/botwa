...
function getDefaultConfig() {
  return {
    global: {
      // Batas ukuran download via yt-dlp (default 1 GB)
...
function getChatConfig(jid) {
  ...
  if (!cfg.chats[jid]) {
    ...
    if (!cfg.chats[jid].features) {
      cfg.chats[jid].features = getDefaultChatConfig().features;
    }
    if (!cfg.chats[jid].stickerPack) {
      cfg.chats[jid].stickerPack = getDefaultChatConfig().stickerPack;
    }
    if (!cfg.chats[jid].prefix) {
      cfg.chats[jid].prefix = getDefaultChatConfig().prefix;
    }
  }
  saveConfig();
