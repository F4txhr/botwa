...
async function handleCommand(sock, msg, ctx) {
  ...
  if (command === "help" || command === "menu") {
    ...
    if (isOwner) {
      ...
      lines.push(
        formatCmd(
          prefix,
          "setlimit &lt;MB&gt;",
          "ubah batas ukuran download global untuk fitur downloader (misal: setlimit 500)",
        ),
      );
      lines.push(
    ...
    lines.push(
      "_Batas default download global untuk fitur downloader adalah ~1 GB (dapat diubah dengan perintah setlimit)._",
    );
  ...
  if (command === "ytv") {
    ...
    try {
      ...
      await sock.sendMessage(
        from,
        {
          text: `Sedang mengambil video, harap tunggu...\n(Info batas ukuran: ~${limitMb} MB)`,
        },
        { quoted: msg },
      );
      const { directUrl, mimetype } = await downloadYtDlpVideo(url, limitBytes);
      await sock.sendMessage(
        from,
        { video: { url: directUrl }, mimetype, caption: "Nih videonya 👍" },
        { quoted: msg },
      );
    } catch (e) {
      console.error("Gagal ambil video via btch-downloader:", e);
      const message = (e && e.message) || "";
      let msgText;
      if (message.includes("Ukuran video melebihi")) {
        msgText = "Gagal mengunduh: ukuran video melebihi batas yang ditetapkan.";
      } else if (message.includes("500 Internal Server Error")) {
        msgText = "Server downloader mengalami gangguan (500). Coba lagi nanti.";
      } else if (message.includes("Respon btch")) {
        msgText = "Gagal mengambil URL video dari layanan downloader. Coba lagi nanti.";
      } else {
        msgText =
          "Gagal mengambil/mengirim video dari URL tersebut. Kemungkinan layanan downloader sedang bermasalah.";
      }
  ...
  if (command === "yta") {
    ...
    try {
      ...
      await sock.sendMessage(
        from,
        {
          text: `Sedang mengambil audio, harap tunggu...\n(Info batas ukuran: ~${limitMb} MB)`,
        },
        { quoted: msg },
      );
      const { directUrl, mimetype } = await downloadYtDlpAudio(url, limitBytes);
      await sock.sendMessage(
        from,
        { audio: { url: directUrl }, mimetype, ptt: false },
        { quoted: msg },
      );
    } catch (e) {
      console.error("Gagal ambil audio via btch-downloader:", e);
      const message = (e && e.message) || "";
      let msgText;
      if (message.includes("Ukuran audio melebihi")) {
        msgText = "Gagal mengunduh: ukuran audio melebihi batas yang ditetapkan.";
      } else if (message.includes("500 Internal Server Error")) {
        msgText = "Server downloader mengalami gangguan (500). Coba lagi nanti.";
      } else if (message.includes("Respon btch")) {
        msgText = "Gagal mengambil URL audio dari layanan downloader. Coba lagi nanti.";
      } else {
        msgText =
          "Gagal mengambil/mengirim audio dari URL tersebut. Kemungkinan layanan downloader sedang bermasalah.";
      }
      await sock.sendMessage(from, { text: msgText }, { quoted: msg });
    }
    return;
  ...
  if (command === "setlimit") {
    ...
    await sock.sendMessage(
      from,
      { text: `Batas download global untuk fitur downloader diubah menjadi ~${mb} MB.` },
      { quoted: msg },
    );
    return;
  }
  ...
  if (command === "status") {
    ...
    lines.push(`📥 *Batas download (global)* : _~${limitMb} MB_`);
  ...
  if (command === "stiker" || command === "sticker") {
    ...
    try {
      ...
      await sock.sendMessage(
        ...
        {
          text:
            "Gagal membuat stiker dari teks. Pastikan dependency sticker (sharp atau wa-sticker-formatter) terinstal dengan benar.",
        },
        { quoted: msg },
      );
    }
...
async function handleToImg(sock, msg, ctx) {
  ...
  try {
    // @ts-ignore baileys types are incorrect, function returns an async iterable
...
async function handleAutoFeatures(sock, msg, ctx) {
  const { from, isGroup, features, stickerPack, trimmedText, lowerText } = ctx;
    ...
    if (url) {
      ...
      if (isYoutube || isTiktok || isInstagram) {
        try {
          ...
          console.error("Gagal ambil video via btch-downloader (auto URL):", e);
          const message = (e && e.message) || "";
          let msgText;
...
async function translateText(text, fromLang, toLang) {
  const res = await axios("https://api.mymemory.translated.net/get", {
    params: {
      q: text,
...
async function shortenUrl(url) {
  const res = await axios("https://is.gd/create.php", {
    params: {
      format: "simple",
      url,
    },
