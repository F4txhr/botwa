...
async function handleCommand(sock, msg, ctx) {
  ...
  if (command === "help" || command === "menu") {
    const helpText = [
      ...
      `${prefix}help / ${prefix}menu - daftar perintah`,
      `${prefix}about - info bot`,
      `${prefix}owner - info pemilik (jika diatur)`,
      "",
      "Downloader:",
      `${prefix}ytv &lt;url&gt; - download video YouTube/TikTok/Instagram`,
      `${prefix}yta &lt;url&gt; - download audio YouTube (mp3)`,
      "Kirim URL YouTube/TikTok/Instagram tanpa command juga akan di-download jika fitur downloader aktif.",
      "",
      "Sticker:",
      `Kirim gambar dengan caption 'stiker' atau 'sticker' → jadi sticker`,
      `${prefix}stiker &lt;teks&gt; - buat sticker teks`,
      `${prefix}stikerpack Nama|Author - set packname &amp; author sticker`,
      "Reply 'stiker' ke gambar → jadi sticker",
      `${prefix}toimg (reply ke stiker) - ubah stiker jadi gambar`,
      "",
      "Utilitas:",
      `${prefix}tr id-en &lt;teks&gt; - translate teks (contoh: ${prefix}tr id-en Halo dunia)`,
      `${prefix}cuaca &lt;kota&gt; - informasi cuaca (butuh OPENWEATHER_API_KEY)`,
      `${prefix}short &lt;url&gt; - pemendek url`,
      "",
      "Admin / Grup:",
      `${prefix}setprefix &lt;prefix&gt; - set prefix untuk chat ini`,
      `${prefix}toggle &lt;fitur&gt; on|off - aktif/nonaktif fitur (downloader,welcome,antilink,translate,weather,shorten,sticker)`,
      `${prefix}status - lihat config untuk chat ini`,
    ].join("\n");
  ...
  if (command === "about") {
    const aboutText = [
      ...
      "- sticker (gambar → sticker, teks → sticker, sticker → gambar)",
      "- translate, cuaca, pemendek URL",
      "",
      "Source code dan konfigurasi dapat dimodifikasi sesuai kebutuhan.",
    ].join("\n");
    ...
    try {
      ...
      const msgText =
        e && e.message && e.message.includes("Ukuran audio melebihi")
          ? "Gagal mengunduh: ukuran audio meleburan audio melebihi batas 1 GB."
  ...
  if (command === "status") {
    ...
    lines.push("");
    lines.push(`Batas download via yt-dlp: ~${limitMb} MB (global)`);
  ...
  if (command === "stiker" || command === "sticker") {
    ...
    try {
      ...
      await sock.sendMessage(
        from,
        { text: "Gagal membuat stiker dari teks. Pastikan sharp atau wa-sticker-formatter terinstal." },
        { quoted: msg },
      );
    }
    return;
  ...
  if (command === "stikerpack") {
    ...
    if (!packname || !author) {
      await sock.sendMessage(
        from,
        {
          text: `Gunakan: ${prefix}stikerpack Nama|Author\nContoh: ${prefix}stikerpack BotWA|Sticker Bot`,
        },
        { quoted: msg },
      );
      return;
...
async function handleMessage(sock, msg, context) {
  ...
  const cmd = parseCommand(trimmedText, prefix);
  if (cmd) {
