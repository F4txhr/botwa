...
const sharp = (() => {
  try {
    ...
    console.warn("sharp tidak tersedia, fitur stiker akan terbatas pada ffmpeg");
    return null;
  }
})();
...
const Sticker = (() => {
  try {
    return require("wa-sticker-formatter").Sticker;
  } catch {
    return null;
  }
})();
...
async function textToSticker(text, packOptions) {
  ...
  if (Sticker && packOptions) {
    ...
    try {
      ...
      console.log("[sticker] Berhasil membuat stiker teks via wa-sticker-formatter, size:", buf.length);
      return buf;
    } catch (e) {
      console.warn(
  ...
  if (sharp) {
    ...
    console.log("[sticker] sharp tidak tersedia, fallback ke ffmpeg drawtext.");
  }
  ...
  console.log("[sticker] Font candidates:", fontCandidates, "dipilih:", fontFile);
  ...
  console.log("[sticker] Menjalankan ffmpeg untuk teks → webp dengan args:", args.join(" "));

  try {
    await runFfmpeg(args);
    console.log("[sticker] ffmpeg selesai tanpa error, membaca output dari:", outPath);
    const outBuf = fs.readFileSync(outPath);
    console.log("[sticker] Berhasil membuat stiker teks via ffmpeg, size:", outBuf.length);
    return outBuf;
  } finally {
    fs.unlink(outPath, () => {});
  }
