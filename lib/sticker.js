...
async function textToSticker(text, packOptions) {
  ...
  // 1) Jika ada sharp, render SVG -> PNG dulu.
  //    Lalu, kalau ada wa-sticker-formatter, gunakan itu dengan buffer PNG
  //    supaya tipe file valid.
  if (sharp) {
    console.log("[sticker] Render teks ke PNG via sharp.");
    const pngBuffer = await sharp(svgBuffer)
      .resize({ width: 512, height: 512, fit: "inside" })
      .png()
      .toBuffer();
    console.log("[sticker] PNG teks siap, size:", pngBuffer.length);

    if (Sticker && packOptions) {
      console.log("[sticker] Menggunakan wa-sticker-formatter untuk stiker teks (dari PNG).");
      try {
        ...
        console.warn("Gagal membuat stiker teks via wa-sticker-formatter (PNG):", e);
      }
    }

    // Jika wa-sticker-formatter tidak tersedia atau gagal,
    // pakai langsung hasil PNG sebagai sticker webp via sharp (fallback lama).
    console.log("[sticker] Menggunakan sharp untuk render teks → webp (fallback).");
  ...
  // 2) Jika tidak ada sharp tapi ada wa-sticker-formatter, coba langsung dari SVG.
  if (Sticker && packOptions) {
    console.log(
      "[sticker] sharp tidak tersedia, mencoba langsung wa-sticker-formatter dengan SVG.",
    );
    try {
      ...
      const buf = await sticker.toBuffer();
      console.log(
        "[sticker] Berhasil membuat stiker teks via wa-sticker-formatter (SVG), size:",
        buf.length,
      );
      return buf;
    } catch (e) {
      console.warn(
        "Gagal membuat stiker teks via wa-sticker-formatter (SVG). Tidak ada engine lain:",
        e,
      );
    }
  }
